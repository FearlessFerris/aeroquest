...
//   useEffect(() => {
//   console.log('GlowBorderParticles MOUNT');
//   return () => console.log('GlowBorderParticles UNMOUNT');
// }, []);
...
export default function GlowBorderParticles({
  ...
  // Rounded-rect clip radius in px
  radiusPx = 36,
  ...
  // ✅ All settings live in a ref, so parent re-render DOES NOT restart loop
  ...
  // Update settings when props change (no restart)
  useEffect(() => {
    settingsRef.current = {
      ...settingsRef.current,
      rgb,
      ...
      sizeRange: Array.isArray(sizeRange) ? sizeRange : [0.8, 1.4],
      speedRange: Array.isArray(speedRange) ? speedRange : [0.35, 1.35],
      lifeRange: Array.isArray(lifeRange) ? lifeRange : [38, 92],
    };
  }, [
    rgb,
    density,
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      const rect = parent.getBoundingClientRect();
      const dpr = clamp(window.devicePixelRatio || 1, 1, s.dprMax);
    ...
    const drawClippedRoundedRect = (ctx2, w, h) => {
      const s = settingsRef.current;
      const radius = clamp(s.radiusPx ?? 36, 0, Math.min(w, h) / 2);
    ...
    const spawnFromPerimeter = (count) => {
      ...
      const pad = s.inset + s.spawnInset;
      const speedMin = s.speedRange[0];
      const speedMax = s.speedRange[1];
      const sizeMin = s.sizeRange[0];
      const sizeMax = s.sizeRange[1];
      const lifeMin = s.lifeRange[0];
      const lifeMax = s.lifeRange[1];
      ...
      for (let i = 0; i < count; i += 1) {
        ...
        const speed = rand(speedMin, speedMax) * s.outward;
        ...
        const vx = nx * speed + tx * tangent * 0.35;
        const vy = ny * speed + ty * tangent * 0.2 + rand(0.02, 0.18) * s.gravity;
    ...
    const step = (now) => {
      ...
      const s = settingsRef.current;
      if (s.paused) return;
      if (s.pauseWhenHidden && hiddenRef.current) return;

      const { w, h } = sizeRef.current;
      if (!w || !h) return;

      // Throttle
      const frameMs = 1000 / clamp(s.fps, 12, 60);
      const t = timeRef.current;
      if (!t.last) t.last = now;
      const delta = now - t.last;
      if (delta < frameMs) return;
      ...
      // Spawn rate
      const perimeter = (w + h) * 2;
      const baseRate = perimeter / 950;
      const spawnCount = Math.floor(clamp(baseRate * s.density, 0.25, 4.5) + Math.random());
      spawnFromPerimeter(spawnCount);

      // Hard cap
      if (particlesRef.current.length > s.maxParticles) {
        particlesRef.current.splice(0, particlesRef.current.length - s.maxParticles);
      }

      const next = [];
      let i = 0;

      const glowBlur = clamp(s.blur, 0, 18);
      const { r, g, b } = s.rgb;
      ...
      for (const p of particlesRef.current) {
        ...
        const t2 = p.age / p.life;
        const alpha = (1 - t2) * 0.95 * s.intensity;
        const tw = 0.75 + Math.sin(p.age * 0.35 + p.tw * 10) * 0.25;

        // Blur only every Nth particle
        const doBlur = glowBlur > 0 && (i % clamp(s.blurEvery, 1, 999) === 0);

        if (doBlur) {
          ctx.filter = `blur(${glowBlur}px)`;
          ctx.fillStyle = `rgba(${r}, ${g}, ${b}, ${alpha * tw * 0.5})`;
          ctx.beginPath();
          ctx.arc(p.x, p.y, p.r * 1.2, 0, Math.PI * 2);
          ctx.fill();
        }
  ...
  return (
    <canvas
      ...
      style={{
        ...
        WebkitMaskImage:
          'radial-gradient(closest-side, rgba(0,0,0,1) 80%, rgba(0,0,0,0) 100%)',
        maskImage:
          'radial-gradient(closest-side, rgba(0,0,0,1) 80%, rgba(0,0,0,0) 100%)',
